...
class HoSoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($gioiTinh === 'nam') {
            // Lấy danh sách ID các loại nghỉ phép chỉ áp dụng cho nữ
            $idsLoaiNghiPhepNu = LoaiNghiPhep::where('gioi_tinh_ap_dung', 'nu')->pluck('id');

            if ($idsLoaiNghiPhepNu->isNotEmpty()) {
                SoDuNghiPhepNhanVien::where('nguoi_dung_id', $user->id)
                    ->whereIn('loai_nghi_phep_id', $idsLoaiNghiPhepNu)
                    ->delete();
            }
        } elseif ($gioiTinh === 'nu') {
            $idsLoaiNghiPhepNam = LoaiNghiPhep::where('gioi_tinh_ap_dung', 'nam')->pluck('id');

            if ($idsLoaiNghiPhepNam->isNotEmpty()) {
                SoDuNghiPhepNhanVien::where('nguoi_dung_id', $user->id)
                    ->whereIn('loai_nghi_phep_id', $idsLoaiNghiPhepNam)
                    ->delete();
            }
        }
