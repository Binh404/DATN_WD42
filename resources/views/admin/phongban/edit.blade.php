...
<div class="container">
    ...
    <form action="/phongban/update/{{ $phongBan->id }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="ten_phong_ban" class="form-control" value="{{ old('ten_phong_ban', $phongBan->ten_phong_ban) }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="ma_phong_ban" class="form-control" value="{{ old('ma_phong_ban', $phongBan->ma_phong_ban) }}">
            @error('ma_phong_ban') <small class="text-danger">{{ $message }}</small> @enderror
        </div>

        <div class="mb-3">
            <label for="mo_ta" class="form-label">Mô tả</label>
            <textarea name="mo_ta" class="form-control">{{ old('mo_ta', $phongBan->mo_ta) }}</textarea>
        ...
        <div class="mb-3">
            ...
            <select name="trang_thai" class="form-control">
                <option value="1" {{ $phongBan->trang_thai == 1 ? 'selected' : '' }}>Kích hoạt</option>
                <option value="0" {{ $phongBan->trang_thai == 0 ? 'selected' : '' }}>Tạm ngưng</option>
            </select>
